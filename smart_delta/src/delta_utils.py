...
UNMARK_MARK = "\\"
SAFE_MARKS = [UNMARK_MARK,  # This has to be the first
    INSERTION_MARK, DELETION_MARK, REPLACEMENT_MARK, REPLACEMENT_SPLIT_MARK, INDEX_PAYLOAD_SEPERATOR_MARK, ]


def replace_signs(data, to_replace, replace_with):
    # print(repr(f"data: {data} | to replace: {to_replace} | replace with: {replace_with}"))
    is_after_mark = False
    new_data = data
