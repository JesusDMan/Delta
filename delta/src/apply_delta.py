...
from backup_syncer.modules.delta.delta import parse_str_delta
...
def apply_string_delta(base_data, delta_string, reverse_delta=False):
    ...
    for delta_step in delta_steps:
        delta = parse_str_delta(delta_step)
        # print("===", data_with_delta, "|", str(delta))
        data_with_delta, offset = apply_delta_step(base_data=data_with_delta,
                                                   delta=delta, reverse_delta=reverse_delta,
                                                   offset=offset)
